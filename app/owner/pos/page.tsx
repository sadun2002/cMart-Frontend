...
export default function POSPage() {
  ...
  const handleCreateCustomer = async (e: React.FormEvent) => {
    ...
    try {
      const res = await saveCustomerLocally(
        newCustomer,
        user?.tenantId || null,
      );
  ...
  useEffect(() => {
    const handleGlobalKeydown = (e: KeyboardEvent) => {
      ...
      if (e.key === "F7" || (e.ctrlKey && e.key.toLowerCase() === "l")) {
        e.preventDefault();
      ...
      if (e.key === "F3" || (e.ctrlKey && e.key.toLowerCase() === "k")) {
        ...
        setFocusedSection("search");
        searchRef.current?.focus();
      } else if (e.key === "F4" || (e.ctrlKey && e.key.toLowerCase() === "c")) {
        e.preventDefault();
        setFocusedSection("chips");
        setFocusedChipIndex((prev) => (prev === -1 ? 0 : prev));
        searchRef.current?.blur();
      } else if (e.key === "F6" || (e.ctrlKey && e.key.toLowerCase() === "p")) {
        e.preventDefault();
  ...
  // Apply 'Recent' logic using actual database sales
  const finalProducts =
    selectedCategory === "Recent" && !search
      ? filteredProducts.filter((p) => recentSoldIds.includes(p.id))
      : filteredProducts;

  // Keyboard Navigation Logic (Arrows/Enter)
  useEffect(() => {
      ...
      if (focusedSection === "search") {
        ...
        if (e.key === "ArrowUp") {
          ...
          setFocusedChipIndex((prev) =>
            Math.min(prev + 1, allChips.length - 1),
          );
        } else if (e.key === "ArrowLeft") {
          e.preventDefault();
          ...
          e.key === "Enter" &&
          focusedChipIndex >= 0 &&
          focusedChipIndex < allChips.length
        ) {
          e.preventDefault();
  ...
  const checkout = async () => {
    ...
    try {
      ...
      setDiscount(0);
      setCustomer(null);
      // Fetch latest recent items after sale
      const recent = await getRecentSoldProductIds(user?.tenantId || null);
      setRecentSoldIds(recent);
    } catch (err: any) {
          ...
          <div className="flex items-center justify-between gap-4 pb-2">
            <div className="flex items-center gap-2 flex-1 min-w-0 relative">
              <div className="flex overflow-x-auto no-scrollbar gap-2 flex-1 py-1 px-2">
                {allChips.map((cat, idx) => {
                  ...
                  const isFocused =
                    focusedSection === "chips" && focusedChipIndex === idx;
            ...
            <div className="flex items-center bg-gray-100 dark:bg-slate-800/80 rounded-lg p-1 flex-shrink-0">
              <button
                ...
                className={`p-1.5 rounded-md transition-colors ${viewMode === "grid" ? "bg-white dark:bg-slate-700 shadow-sm text-blue-600 dark:text-blue-400" : "text-gray-500 dark:bg-slate-800 dark:text-slate-400 hover:text-gray-700 dark:hover:text-slate-200"}`}
              >
                <Grid className="w-4 h-4" />
              </button>
              <button
            ...
            <div id="product-grid" className="grid grid-cols-[repeat(auto-fill,minmax(130px,1fr))] gap-4">
              {finalProducts.map((p, idx) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`h-32 w-full bg-gradient-to-br ${color} flex items-center justify-center relative overflow-hidden`}
                    >
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between p-3">
                      ...
                      >
                        <div
                          className={`w-14 h-14 rounded-xl bg-gradient-to-br ${color} flex flex-shrink-0 items-center justify-center cursor-pointer overflow-hidden`}
                        >
                          {p.images && p.images.length > 0 ? (
                          ...
                          <div className="p-4 grid grid-cols-3 gap-4 text-sm">
                            ...
                            {(p.brand || p.color || p.size) && (
                              <div className="col-span-2 flex gap-4">
                                {p.brand && (
                                  <div>
                                    <span className="text-gray-400 dark:text-slate-500 text-xs block">
                                      Brand
                                    </span>
                                    {p.brand}
                                  </div>
          ...
          {finalProducts.length === 0 && (
            <div className="font-sans h-full flex flex-col items-center justify-center text-center opacity-50">
              ...
              <p className="text-sm text-gray-500 dark:text-slate-400 mt-2">
                Try scanning a different barcode or change the category.
              </p>
            </div>
          )}
        </div>
      ...
      <div className="w-[400px] flex-shrink-0 flex flex-col bg-white dark:bg-slate-900 shadow-[-10px_0_30px_rgba(0,0,0,0.02)] dark:shadow-none z-10">
        ...
        <div className="p-4 bg-gray-50 dark:bg-slate-900 border-t border-gray-100 dark:border-slate-800 space-y-4">
          ...
          <div className="flex justify-between items-end pt-3 border-t border-gray-200 dark:border-slate-700">
            ...
            <span className="text-2xl font-black text-blue-600 dark:text-blue-400 tracking-tight truncate ml-2 text-right">
              {formatLKR(total)}
            </span>
          </div>
        ...
        {customerModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
            ...
            <motion.div
              ...
              className="relative w-full max-w-lg bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-gray-100 dark:border-slate-800 overflow-hidden flex flex-col max-h-[90vh]"
            >
                  ...
                  <form onSubmit={handleCreateCustomer} className="space-y-4">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full p-3 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 dark:text-white"
                    ...
                    <div className="flex gap-3 mt-6">
                      <button
                        ...
                        className="flex-1 px-4 py-3 bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-slate-300 font-bold rounded-xl hover:bg-gray-200 dark:hover:bg-slate-700 transition-colors"
                      >
                        Cancel
                      </button>
                      <button
                        type="submit"
                        className="flex-1 px-4 py-3 bg-blue-600 text-white font-bold rounded-xl shadow-lg shadow-blue-500/25 hover:bg-blue-700 transition-colors"
                      >
                        Save Customer
                      </button>
                    </div>
                  ...
                  <>
                    <div className="relative mb-6">
                      ...
                      <input
                        ...
                        className="w-full pl-11 pr-4 py-3 bg-gray-50 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 dark:text-white transition-shadow"
                      />
                    </div>
                    ...
                    <div className="space-y-2 max-h-60 overflow-y-auto min-h-[150px]">
                      <button
                        ...
                        className="w-full p-3 text-left bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 rounded-xl hover:border-emerald-500 dark:hover:border-emerald-500 transition-colors flex items-center justify-between group"
                      >
                        <div className="flex items-center gap-3">
                      ...
                      {customersList.map((c) => (
                        <button
                          ...
                          className="w-full p-3 text-left bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 rounded-xl hover:border-blue-500 dark:hover:border-blue-500 transition-colors flex items-center justify-between group"
                        >
                          <div className="flex items-center gap-3">
                    ...
                    <button
                      onClick={() => router.push("/owner/customers?action=add")}
                      className="w-full mt-4 flex items-center justify-center gap-2 px-4 py-3 border-2 border-dashed border-gray-200 dark:border-slate-700 rounded-xl text-gray-600 dark:text-slate-400 font-bold hover:border-blue-500 hover:text-blue-600 dark:hover:border-blue-500 dark:hover:text-blue-400 transition-colors"
                    >
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="w-2/5 bg-gray-50 dark:bg-slate-800/50 p-8 border-r border-gray-100 dark:border-slate-800 flex flex-col justify-between">
                ...
                <div>
                  <p className="text-sm font-bold text-gray-400 dark:text-slate-500 uppercase tracking-wider mb-2">
                    Total Due
                  </p>
                  <p className="text-2xl font-black text-gray-900 dark:text-white tracking-tight break-words">
                    {formatLKR(total)}
                  </p>
                </div>
              </div>
        ...
        {isHelpModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 20 }}
              className="relative w-full max-w-md bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-gray-100 dark:border-slate-800 overflow-hidden"
            >
              <div className="p-6 border-b border-gray-100 dark:border-slate-800 flex justify-between items-center bg-blue-50 dark:bg-slate-800/50">
                ...
                <button
                  onClick={() => setIsHelpModalOpen(false)}
                  className="p-2 bg-blue-100 dark:bg-slate-800 rounded-full text-blue-500 hover:text-blue-700 transition-colors"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
              <div className="p-6 space-y-4 max-h-[300px] overflow-y-auto">
                {[
                  ...
                  <div
                    key={hk.key}
                    className="flex justify-between items-center p-3 bg-gray-50 dark:bg-slate-800/50 rounded-xl border border-gray-100 dark:border-slate-700/50"
                  >
                    <span className="font-semibold text-gray-700 dark:text-slate-300">
                      {hk.desc}
                    </span>
                    <span className="px-3 py-1 bg-white dark:bg-slate-700 rounded-lg shadow-sm border border-gray-200 dark:border-slate-600 text-sm font-black text-gray-900 dark:text-white">
                      {hk.key}
                    </span>
                  </div>
                ))}
              ...
              <div className="p-6 pt-2">
                <button
                  onClick={() => setIsHelpModalOpen(false)}
                  className="w-full py-3 bg-blue-600 text-white font-bold rounded-xl shadow-lg shadow-blue-500/25 hover:bg-blue-700 transition-colors"
                >
                  Got It!
                </button>
              </div>
